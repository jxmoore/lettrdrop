...
/* ── The falling well ── */
function DropBoard({ grid, cols = 6, active = null, ghostCell = null, matched = [], clearCls = '', shuffling = false, rocking = false, swapping = false }) {
...
function DropInGame({
  ...
  matched = [], clearCls = '', shuffling = false, rocking = false, swapping = false, next = ['Q', 'U', 'A'],
  score = '1,240', words = '18', best = '3,910', scoreGlow = false, overlay = null,
}) {
  return (
    <div className="pscreen">
      <DTopBar jumbles={jumbles} swaps={swaps} />
      <DStats score={score} words={words} best={best} scoreGlow={scoreGlow} />
      <PlayStrip next={next} />
      <DropBoard grid={grid} active={active} ghostCell={ghost} matched={matched} clearCls={clearCls} shuffling={shuffling} rocking={rocking} swapping={swapping} />
      <PlayHint />
      {overlay}
    </div>
  );
